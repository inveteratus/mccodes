...
require __DIR__ . '/../include/class_bbcode.php';
...
&gt; <a href="forums.php">Back</a>
   ';
    ...
    $db->free_result($q3);
    if ($ir['userid'] == $post['fp_poster_id'])
    {
        echo '
...
function get_links(array $r, int &$no): array
{
    ...
    $elink = $ir['userid'] == $r['fp_poster_id'] ? "[<a href='forums.php?act=edit&amp;post={$r['fp_id']}&amp;topic={$_GET['viewtopic']}'>Edit Post</a>]" : '';
    $no++;
    $dlink = ($no > 1) ? "[<a href='forums.php?act=delepost&amp;post={$r['fp_id']}'>Delete Post</a>]" : '';
